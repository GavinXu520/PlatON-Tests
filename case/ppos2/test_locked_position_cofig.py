...
class TestLockeDpositionConfig:
    ...
    def test_unlock_Normal(self):
        ...
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == loukupbalace, "锁仓金额：{}有误".format(
            dict_Info['balance'])
        ...
        dict_Info = json.loads(RestrictingInfo['Data'])
        balance = platon_ppos.eth.getBalance(address2)
        assert dict_Info['balance'] == 0, "锁仓金额：{}有误".format(
            dict_Info['balance'])
    ...
    def test_multiple_unlock_Normal(self):
        ...
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == loukupbalace * 2, "锁仓金额：{}有误".format(
            dict_Info['balance'])
        ...
        assert dict_Info['balance'] == loukupbalace, "锁仓金额：{}有误".format(
            dict_Info['balance'])
        assert dict_Info['Entry'][0][
            'amount'] == loukupbalace, "第二个解锁期待释放金额：{}".format(
                dict_Info['Entry'][0]['amount'])
        assert balance2 == loukupbalace, "返回的释放锁仓金额：{} 有误".format(balance)
    ...
    def test_unlock_point_pledge_amount(self):
        ...
        assert result['Status'] == True, "创建锁仓计划返回的状态：{},用例失败".format(
            result['Status'])
        RestrictingInfo = platon_ppos.GetRestrictingInfo(address2)
        assert RestrictingInfo['Status'] == True, "查询锁仓计划返回的状态：{},用例失败".format(
            result['Status'])
        info = json.loads(RestrictingInfo['Data'])
        assert info['balance'] == loukupbalace, "锁仓计划可用余额：{},有误".format(
            info['balance'])
        ...
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == Web3.toWei(lockupamoutn - amount,
                                                  'ether'), "锁仓金额：{}有误".format(
                                                      dict_Info['balance'])

        # 到达解锁期之后锁仓账号余额
        CommonMethod.get_next_settlement_interval(self)
        balance4 = platon_ppos.eth.getBalance(address2)
        log.info("到达解锁期后预期账户余额：{}".format(balance3 +
                                          Web3.toWei(lockupamoutn -
                                                     amount, 'ether')))
        log.info("到达解锁期后实际账户余额：{}".format(balance4))
        assert balance4 == balance3 + Web3.toWei(
            lockupamoutn - amount, 'ether'), "锁仓账户金额：{} 有误".format(balance4)
        RestrictingInfo = platon_ppos.GetRestrictingInfo(address2)
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == 0, "锁仓金额：{}有误".format(
            dict_Info['balance'])
        assert dict_Info['symbol'] == True, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == Web3.toWei(
            amount, 'ether'), "欠释放锁仓金额：{} 有误".format(dict_Info['debt'])
        ...
        assert balance6 == balance5 + loukupbalace, "锁仓账户金额：{} 有误".format(
            balance6)
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == 0, "锁仓金额：{}有误".format(
            dict_Info['balance'])
        assert dict_Info['symbol'] == False, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
    ...
    def test_unlock_point_delegtion_notbalance(self):
        ...
        assert RestrictingInfo['Status'] == True, "查询锁仓计划返回的状态：{},用例失败".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == loukupbalace, "锁仓计划金额：{}有误".format(
            dict_Info['balance'])
        assert dict_Info['symbol'] == False, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == 0, "欠释放锁仓金额：{} 有误".format(
        ...
        assert result['Status'] == True, "申请委托返回的状态：{},用例失败".format(
            result['Status'])
        balance4 = platon_ppos.eth.getBalance(address2)
        ...
        assert dict_Info['symbol'] == True, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == loukupbalace, "欠释放锁仓金额：{} 有误".format(
            dict_Info['debt'])
        ...
        assert balance7 == balance6 + Web3.toWei(
            partamount, 'ether'), "到达解锁期赎回部分委托后账户余额：{}".format(balance7)
        RestrictingInfo = platon_ppos.GetRestrictingInfo(address2)
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        ...
        assert dict_Info['symbol'] == True, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == loukupbalace - Web3.toWei(
            partamount, 'ether'), "欠释放锁仓金额：{} 有误".format(dict_Info['debt'])
        #申请赎回全部委托金
        delegate_info = platon_ppos.unDelegate(stakingBlockNum,
                                               nodeId,
                                               lockupamoutn - partamount,
                                               privatekey=private_key2,
                                               from_address=address2,
                                               gasPrice=self.base_gas_price,
                                               gas=self.staking_gas)
        ...
        log.info("{}赎回全部委托金后预期账户余额：{}".format(
            address2,
            balance8 + Web3.toWei(lockupamoutn - partamount, 'ether')))
        assert balance9 == balance8 + loukupbalace - Web3.toWei(
            partamount, 'ether'), "到达解锁期赎回委托后账户余额：{}".format(balance9)
        RestrictingInfo = platon_ppos.GetRestrictingInfo(address2)
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
    ...
    def test_unlock_point_pledge_punish_amount(self):
        ...
        assert result['Status'] == True, "获取质押节点返回状态为:{} 有误".format(
            result['Status'])
        RestrictingPlan = result['Data']['RestrictingPlanHes']
        assert RestrictingPlan == Web3.toWei(
            amount, 'ether'), '质押金额：{} 有误'.format(RestrictingPlan)
        Balance3 = platon_ppos.eth.getBalance(address2)
        ...
        CommonMethod.get_next_settlement_interval(self, 1)
        Balance5 = platon_ppos.eth.getBalance(address2)
        log.info("{}到达解锁期后处罚节点后预期账户余额：{}".format(
            address2, Balance4 + Web3.toWei(amount - (amount * 0.2), 'ether')))
        log.info("{}到达解锁期后处罚节点后实际账户余额：{}".format(address2, Balance5))
        result = platon_ppos.getCandidateInfo(nodeId)
        assert Balance5 == Balance4 + Web3.toWei(
            amount - (amount * 0.2), 'ether'), "锁仓账户金额：{} 有误".format(Balance4)
        RestrictingInfo = platon_ppos.GetRestrictingInfo(address2)
        assert RestrictingInfo['Status'] == True, "获取锁仓计划返回状态为:{} 有误".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == 0, "锁仓计划可用金额：{}有误".format(
            dict_Info['balance'])
        assert dict_Info['symbol'] == True, "欠释放状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == Web3.toWei(
            amount - (amount * 0.2),
            'ether'), "欠释放锁仓金额：{} 有误".format(dict_Info['debt'])
    ...
    def test_eliminated_verifier_create_lockup(self):
        ...
        result = platon_ppos.createStaking(1,
                                           ...
                                           gasPrice=self.base_gas_price,
                                           gas=self.staking_gas)
        assert result['Status'] == False, "申请质押返回的状态：{},用例失败".format(
            result['Status'])
    ...
    def test_owe_amountstack_lock_plan(self):
        ...
        assert RestrictingInfo['Status'] == True, "查询锁仓计划返回的状态：{},用例失败".format(
            result['Status'])
        dict_Info = json.loads(RestrictingInfo['Data'])
        assert dict_Info['balance'] == Web3.toWei(
            100, 'ether'), "锁仓金额：{}有误".format(dict_Info['balance'])
        assert dict_Info['symbol'] == True, "锁仓的状态：{} 有误".format(
            dict_Info['symbol'])
        assert dict_Info['debt'] == Web3.toWei(
